...
class LoggerQgis:
    ...
    def log_msg(self, massage, module_tag, level=Qgis.Info):
        logfile_path = os.path.join(self.current_project_path, os.sep, 'rousettus.log')
        try:
            with open(logfile_path, 'a') as logfile:
                logfile.write('{}\t{}\t{}\t{}\n'.format(datetime.datetime.now().strftime("%d.%m.%YT%H:%M:%S.S"),
                                                        module_tag,
        ...
        QgsMessageLog.logMessage("{}. {}".format(module_tag, massage), 'Rousettus_Tool', level=module_tag)